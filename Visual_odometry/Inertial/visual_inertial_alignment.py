...
from Inertial.optimize_scale_gravity_velocity import solve_scale_gravity_velocity
...
class VisualInertialAlignment:
    ...
    def run_between(
        ...
        for i in range(n_pairs):
            ...
            chunks = self._chunk_db.get_chunks_between(t_start, t_end, tolerance=tolerance)
            if not chunks:
                raise RuntimeError(f"[VIA] No IMU chunks found between {t_start:.3f} and {t_end:.3f}.")
            ...
            if not all_raw:
                raise RuntimeError(f"[VIA] No raw IMU data between {t_start:.3f} and {t_end:.3f}.")
    ...
    def run(
        ...
        dts = [sum(d for d, _, _ in seg) for seg in imu_segments]

        # Step 2: Gyro bias + re-preintegrate
        visual_quats = [pose_to_quat(R) for _, R in keyframe_poses]
        delta_bw = calibrate_gyro_bias(visual_quats, gammas, J_gamma_bws)
        self.b_w += delta_bw

        preint_results = [preintegrate(seg, self.b_a, self.b_w) for seg in imu_segments]
        alphas = [r[0] for r in preint_results]
        betas  = [r[1] for r in preint_results]
        J_alpha_bas = [r[3] for r in preint_results]
        J_beta_bas  = [r[5] for r in preint_results]

        # Step 3: Joint solve (scale + gravity + velocity + b_a)
        s, g_c0, velocities, delta_ba = solve_scale_gravity_velocity(
            keyframe_poses, alphas, betas, dts, self.p_bc, self.R_bc,
            J_alpha_bas, J_beta_bas
        )

        self.b_a += delta_ba

        # Step 4: Re-preintegrate with corrected b_a + refine gravity
        preint_results = [preintegrate(seg, self.b_a, self.b_w) for seg in imu_segments]
        alphas = [r[0] for r in preint_results]
        betas  = [r[1] for r in preint_results]

        g_c0 = refine_gravity(g_c0, keyframe_poses, alphas, betas, dts, self.p_bc, velocities)
