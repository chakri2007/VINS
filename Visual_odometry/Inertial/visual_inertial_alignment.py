...
class VisualInertialAlignment:
    ...
    def run(
        ...
        [BUGFIX] self.b_a was previously fixed at zero forever (only b_w
        was corrected each run). Real accel bias leaked into alpha/beta
        uncorrected, and the leak grows with window duration -- this
        produced the slow, monotonic gravity-magnitude drift seen in the
        field log. solve_scale_gravity_velocity() now jointly estimates a
        delta_b_a using the J_a_ba/J_v_ba Jacobians preintegrate() already
        computes (previously unused), and that correction is applied to
        self.b_a below, the same way delta_bw is applied to self.b_w.
        """
        ...
        # Re-preintegrate with corrected gyro bias (b_a still at its prior
        # estimate here — preintegrate() linearizes around b_a=self.b_a,
        # giving fresh J_a_ba/J_v_ba Jacobians for the b_a solve below)
        preint_results = [
            preintegrate(seg, self.b_a, self.b_w) for seg in imu_segments
        ]
        alphas   = [r[0] for r in preint_results]
        betas    = [r[1] for r in preint_results]
        J_a_bas  = [r[3] for r in preint_results]   # d(alpha)/d(b_a)
        J_v_bas  = [r[5] for r in preint_results]   # d(beta)/d(b_a)

        # ── Step 3: Solve for scale, gravity, velocities, AND b_a ─────────
        s, g_c0, velocities, delta_b_a = solve_scale_gravity_velocity(
            keyframe_poses, alphas, betas, dts, self.p_bc, self.R_bc,
            J_a_bas=J_a_bas, J_v_bas=J_v_bas,
        )
        ...
        # Apply the accel bias correction (mirrors self.b_w += delta_bw above)
        self.b_a += delta_b_a

        # ── Step 4: Gravity refinement ────────────────────────────────────
        # [BUGFIX] Re-preintegrate once more with the corrected self.b_a —
        # otherwise refine_gravity() would run on the stale alphas/betas
        # from before the bias correction, undoing part of the fix above.
        preint_results = [
            preintegrate(seg, self.b_a, self.b_w) for seg in imu_segments
        ]
        alphas = [r[0] for r in preint_results]
