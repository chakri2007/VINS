...
from optimization.imu_factor import robust_sqrt_information
...
def _imu_sqrt_information(covariance, eps=None):
    """
    Dense 15x15 sqrt-information from the preintegration's error-state
    covariance: L such that L @ L.T == inv(covariance).

    Delegates to optimization.imu_factor.robust_sqrt_information (see
    that docstring for why a scale-relative eigenvalue floor is used
    instead of a fixed absolute ridge -- the fixed-eps version
    previously here had the same systemic issue documented there:
    it pinned the information matrix's worst-direction eigenvalue at
    ~1/eps regardless of how well-conditioned the interval actually
    was, which was directly responsible for the 1e6-1e15 initial costs
    and NO_CONVERGENCE results seen in both BA_motion and the windowed
    BA. `eps` is accepted and ignored for backwards compatibility with
    any existing call sites that pass it explicitly.
    """
    return robust_sqrt_information(covariance)
