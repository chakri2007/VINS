...
def bundle_adjustment_motion(
    ...
    # Was max_iterations=10, max_solver_time_in_seconds=0.04 ("VINS-Mono
    # steady-state BA_motion cap"). Production logs show this budget was
    # never actually time-bound in practice -- BA_motion's problem is
    # tiny (1 pose/velocity/bias node, typically 10-25 observations), so
    # per-iteration cost is sub-millisecond and 0.04s was never the
    # binding constraint. max_iterations=10 was: every single
    # NO_CONVERGENCE rejection in production logs reports exactly
    # "Number of iterations: 10" (11 counting iteration 0), regardless
    # of how far the cost had already dropped by then -- i.e. the
    # iteration cap, not the time cap or a genuine local-minimum
    # plateau, was cutting these solves off.
    #
    # This mirrors the identical fix already applied to the windowed BA
    # (see vio_core.py's baWindowMaxSolverTimeSeconds / run_windowed_
    # optimization's max_iterations=30): raise the iteration ceiling to
    # something LM actually needs to satisfy its OWN convergence
    # tolerances, since the real-time budget was never the bottleneck.
    #
    # IMPORTANT -- this does NOT fix the separate, more serious pattern
    # also visible in the same production logs: a chain of
    # NO_CONVERGENCE rejections whose *initial* cost grows by many
    # orders of magnitude between consecutive frames (e.g. 1e15 ->
    # 1e21 -> 1e28), eventually with 0 usable correspondences at all.
    # That is BA_motion being handed an already-catastrophic pose guess,
    # not an under-budgeted solver -- it is the visible signature of
    # visual_inertial_optimization's own IMU-only fallback
    # (commit_imu_fallback in vio_core.py) compounding: each rejected
    # frame falls back to pure double-integrated dead reckoning with NO
    # vision correction, which then seeds the *next* frame's PnP/
    # BA_motion guess, which is now further off, produces worse/fewer
    # correspondences, and is more likely to fail again. Raising the
    # iteration budget here helps BA_motion actually converge on the
    # *recoverable* cases (modest cost, previously just cut off early)
    # and should shrink how often the fallback chain gets triggered in
    # the first place, but a long enough loss of visual tracking will
    # still eventually produce a genuinely unrecoverable pose guess that
    # no amount of solver budget fixes -- see
    # visual_inertial_optimization's consecutive-fallback tracking in
    # vio_core.py for a mitigation of THAT problem, not this one.
    max_iterations=30,
    huber_delta=1.0,
    num_threads=4,
    max_solver_time_in_seconds=0.15,
    verbose=False,
):
    """
